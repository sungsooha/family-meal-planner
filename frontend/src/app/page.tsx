...
import { buildCalendar, dayTileLabel, formatLocalDate } from "@/lib/calendar";
...
import { useDailyRecommendations } from "@/lib/useDailyRecommendations";
import DailyRecommendationsModal from "@/components/DailyRecommendationsModal";
...
export default function WeeklyPlanPage() {
  ...
  const { recipes, recipesById, optimisticIds, mutateRecipes } = useRecipes();
  ...
  const { data: activeRecipeData } = useSWR<RecipeDetailResponse | null>(
  ...
  // daily recommendation state now managed by useDailyRecommendations
  ...
  useEffect(() => {
    fetch("/api/plan/dates")
      .then((res) => res.json())
      .then((data: PlanDatesResponse) => setPlannedDates(new Set(data.dates ?? [])))
  ...
  const scheduleFeedbackSave = (recipeId: string, feedback: Record<string, number>) => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const response = await fetch(`/api/recipes/${recipeId}/feedback`, {
        ...
        body: JSON.stringify({ family_feedback: feedback } satisfies RecipeFeedbackRequest),
      });
      if (!response.ok) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium shadow-sm ${
              daily.dailyRuns.length
                ? "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100"
                : "border-slate-200 bg-white text-slate-700 hover:text-slate-900"
            }`}
            onClick={() => {
              if (daily.dailyRuns[0]) {
                daily.setDailyActiveRunId(daily.dailyRuns[0].id);
              }
              daily.setDailyModalOpen(true);
            }}
          >
          ...
          <ActionMenu>
            ...
            <button
              className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-700 hover:text-slate-900"
              onClick={() => daily.handleGenerateDaily()}
              disabled={daily.dailyLoading}
            >
              <Wand2 className="h-4 w-4" />
              {daily.dailyLoading ? "Generating daily…" : "Daily recommendations"}
            </button>
            <button
              className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-700 hover:text-slate-900"
              ...
              .map((day) => {
                ...
                return (
                  ...
                  >
                    ...
                    const isPending = !!daily.pendingAssignments[`${day.date}:${meal}`];
                    return (
                      <div
                        key={meal}
                      ...
                      >
                            ...
                            {isFeedbackOpen && entry?.recipe_id && recipeMeta ? (
                              ...
                              >
                                <FamilyFeedback
                                  members={members}
                                  feedback={recipeMeta.family_feedback ?? undefined}
      ...
      {daily.dailyModalOpen && (
        <DailyRecommendationsModal
          open={daily.dailyModalOpen}
          onClose={() => daily.setDailyModalOpen(false)}
          dailyRuns={daily.dailyRuns}
          activeDailyRun={daily.activeDailyRun}
          setDailyActiveRunId={daily.setDailyActiveRunId}
          dailyLoading={daily.dailyLoading}
          dailyLoadingStep={daily.dailyLoadingStep}
          dailyLoadingSteps={daily.dailyLoadingSteps}
          dailyProgress={daily.dailyProgress}
          dailyMessage={daily.dailyMessage}
          dailyRunErrors={daily.dailyRunErrors}
          dailyCandidateErrors={daily.dailyCandidateErrors}
          dailyCandidates={daily.dailyCandidates}
          dailySimilarMap={daily.dailySimilarMap}
          dailyVideoCandidateId={daily.dailyVideoCandidateId}
          setDailyVideoCandidateId={daily.setDailyVideoCandidateId}
          dailyAssignCandidateId={daily.dailyAssignCandidateId}
          setDailyAssignCandidateId={daily.setDailyAssignCandidateId}
          dailyAssignMeal={daily.dailyAssignMeal}
          setDailyAssignMeal={daily.setDailyAssignMeal}
          dailyAssignDate={daily.dailyAssignDate}
          ...
          handleDailyDeleteRun={daily.handleDailyDeleteRun}
          buildCalendar={buildCalendar}
          dayTileLabel={dayTileLabel}
          formatLocalDate={formatLocalDate}
          mealTypeOptions={mealTypeOptions}
          planDays={plan?.days}
          recipesById={recipesById}
          language={language}
          showDailyDebug={showDailyDebug}
        />
      )}
          ...
          <div className="mt-6 space-y-4">
            ...
            <div>
              ...
              <div className="mt-2">
                <FamilyFeedback
                  members={members}
                  feedback={activeRecipe.family_feedback ?? undefined}

...
## Caching & Data Reuse
- Seed recipe detail cache from the recipe list so `/recipes/[id]` can render instantly. (implemented)
- Provide a shared `useRecipes()` hook that exposes a cached list + `recipesById` map. (implemented)
